...
def main():
    parser = argparse.ArgumentParser(
        description="Random Forest regression for Dmax prediction and test-set R2/RMSE/MAE (evaluation only)."
    )
    ...
    parser.add_argument("--max_features", type=str, default="sqrt",
                        help="Number of features to consider when looking for the best split: "
                             "sqrt/log2/auto/None or a float ratio in [0, 1].")
    parser.add_argument("--bootstrap", action="store_true",
                        help="Whether to use bootstrap sampling (default: off).")
    ...
    # Parse max_features: allow a float ratio in [0, 1]
    mf = args.max_features
    try:
        if mf is not None and re.fullmatch(r"\d*\.?\d+", str(mf)):
            mf = float(mf)
    except Exception:
        pass
    ...
    model = RandomForestRegressor(
        ...
        n_jobs=args.n_jobs,
        random_state=0,  # for reproducibility only
    )
    model.fit(Xtr, ytr)
    ...
    print(f"n_estimators={args.n_estimators} max_depth={args.max_depth} "
          ...
          f"max_features={args.max_features} bootstrap={args.bootstrap}")
