...
try:
    ...
    raise ImportError(
        "lightgbm is not installed: please run `pip install lightgbm` or install it via conda."
    ) from e
...
def main():
    parser = argparse.ArgumentParser(
        description="LightGBM regression for Dmax prediction and test-set R2/RMSE/MAE (evaluation only)."
    )
    ...
    parser.add_argument("--n_estimators", type=int, default=3000,
                        help="Number of boosting iterations / trees.")
    parser.add_argument("--learning_rate", type=float, default=0.02,
                        help="Learning rate.")
    parser.add_argument("--num_leaves", type=int, default=63,
                        help="Number of leaves (controls model complexity).")
    ...
    parser.add_argument("--subsample", type=float, default=0.8,
                        help="Row subsampling ratio (bagging_fraction).")
    parser.add_argument("--colsample_bytree", type=float, default=0.8,
                        help="Column subsampling ratio (feature_fraction).")
    ...
    print("[LightGBM]")
    print(f"n_estimators={args.n_estimators} lr={args.learning_rate} num_leaves={args.num_leaves} "
