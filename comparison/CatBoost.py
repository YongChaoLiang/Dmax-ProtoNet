...
try:
    ...
    raise ImportError("catboost is not installed: please run `pip install catboost` first.") from e
...
def pick_element_cols(df: pd.DataFrame, target_col: str):
    ...
    and the column must be numeric.
    """
    ...
    if len(cols) == 0:
        raise ValueError(
            ...
            "Please ensure the feature columns are named as chemical element symbols."
        )
    return cols
...
def main():
    parser = argparse.ArgumentParser(
        description="CatBoost regression for Dmax prediction and test-set R2/RMSE/MAE (evaluation only)."
    )
    ...
    parser.add_argument("--iterations", type=int, default=1000,
                        help="Number of boosting iterations (max trees).")
    ...
    parser.add_argument("--loss_function", type=str, default="RMSE",
                        help="Loss function (e.g., RMSE/MAE/Huber).")
    parser.add_argument("--bootstrap_type", type=str, default="Bayesian",
                        choices=["Bayesian", "Bernoulli", "MVS"],
                        help="Bootstrap sampling type.")
    parser.add_argument("--subsample", type=float, default=None,
                        help="Row sampling rate for Bernoulli/MVS (e.g., 0.8). Usually leave empty for Bayesian.")
    parser.add_argument("--rsm", type=float, default=0.8,
                        help="Column sampling rate (random subspace method).")
    parser.add_argument("--verbose", type=int, default=200,
                        help="Logging frequency (0 for silent).")
    ...
    # "subsample" is only effective for certain bootstrap types
    fit_params = {}
    if args.subsample is not None:
        fit_params["subsample"] = float(args.subsample)
    ...
    model = CatBoostRegressor(
        ...
        random_seed=42,              # for reproducibility only
        allow_writing_files=False,   # avoid writing logs to the working directory
    )
    ...
    print("[CatBoost]")
    print(f"iters={args.iterations} lr={args.learning_rate} depth={args.depth} "
          f"loss={args.loss_function} bootstrap={args.bootstrap_type} rsm={args.rsm}")
    if args.subsample is not None:
