...
class MatProtoFeaturizer(nn.Module):
    """Learnable property-primitive matrix + derived global statistics + pairwise bias and Laplacian regularization."""
...
class MatBiasSelfAttention(nn.Module):
    ...
    def forward(self, x, bias_ij: torch.Tensor, key_mask: torch.Tensor = None):
        ...
        Args:
            x:        [B, N, D]
            bias_ij:  [N, N]  (already expanded to include token0 if used)
...
class SEBlock(nn.Module):
    """Squeeze-and-Excitation block operating over the token dimension."""
    ...
    def forward(self, x):
        ...
        s = self.sig(s).unsqueeze(1)
        return x * s


class MatPhys_GA(nn.Module):
    """Composition-only regression model with property primitives and bias-aware self-attention."""
    ...
    def __init__(
        ...
        self.ctx_proj = nn.Linear(2 * K_proto + 2, model_dim)
    ...
    def forward(self, x):
        ...
        elem_tok = self.elem(idx) + self.frac(x.unsqueeze(-1))                         # [B, N, D]

        stats = self.proto.global_stats(x)                  # [B, 2K+2]
        ctx_tok = self.ctx_proj(stats).unsqueeze(1)         # [B, 1, D]

        tok = torch.cat([ctx_tok, elem_tok], dim=1)         # [B, N+1, D]

        bias_ij = self.proto.pair_bias().to(x.device)       # [N, N]
        bias_full = torch.zeros((self.N + 1, self.N + 1), device=x.device, dtype=bias_ij.dtype)
        bias_full[1:, 1:] = bias_ij
        ...
        tok = tok + self.attn(self.norm(tok), bias_full, key_mask=key_mask)
